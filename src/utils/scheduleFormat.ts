...
export function describeFrequency(
  ...
  if (frequency === 'biweekly') return `Every 2 weeks on ${DAYS_OF_WEEK[dayOfWeek]}`;
...
export function formatScheduleSummary(parts: ScheduleParts): string {
  ...
  const first = firstRunDate({ frequency, dayOfWeek, dayOfMonth, startDate, endDate });
...
export function getNextBillDates(parts: ScheduleParts, count: number): string[] {
  const { frequency, dayOfWeek, dayOfMonth, startDate, endDate } = parts;
  return nextRunDates({ frequency, dayOfWeek, dayOfMonth, startDate, endDate }, count).map((d) =>
...
export function scheduleHasOccurrences(parts: ScheduleParts): boolean {
  ...
  return nextRunDates({ frequency, dayOfWeek, dayOfMonth, startDate, endDate }, 1).length > 0;
}
