...
export function ItemAssignmentDropdown({ item, people, itemAssignments, onAssign, onAssignAll, showSplit = false }: Props) {
  const displayNames = useMemo(() => getAbbreviatedNames(people), [people]);
  const allAssigned = (itemAssignments[item.id] || []).length === people.length && people.length > 0;
      ...
      <Popover>
        ...
        <PopoverContent className="w-64 p-0">
          <div className="p-2 space-y-1">
            {onAssignAll && people.length > 1 && (
              <div
                className="flex items-center space-x-3 px-3 py-2.5 rounded-md cursor-pointer transition-all hover:bg-secondary border-b mb-1 pb-2"
                onClick={() => onAssignAll(item.id)}
              >
                <div
                  className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-all ${
                    allAssigned ? 'bg-primary border-primary' : 'border-input'
                  }`}
                >
                  {allAssigned && <Check className="w-3 h-3 text-primary-foreground" />}
                </div>
