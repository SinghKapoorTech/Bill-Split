...
export function AirbnbReviewStep({
  ...
  // Must be declared BEFORE the `!billData` early return below: a hook placed
    ...
    try {
      ...
      toast({
        ...
        description: isSettled
          ? 'Their balance has been updated to $0 for this trip.'
          : 'Their balance has been restored.',
      });
    } catch (error) {
    ...
    <div className="flex flex-col gap-6 fade-in max-w-xl mx-auto w-full pb-20">
      ...
      {!allItemsAssigned && (
        <Alert variant="destructive" className="bg-destructive/10 text-destructive border-none">
          <AlertDescription>
            Not all nights or fees are fully assigned. The totals below might not equal the grand
            total.
            <Button
              variant="link"
              className="p-0 h-auto ml-2 text-destructive font-bold"
              onClick={onPrev}
            >
              Go back to fix
            </Button>
