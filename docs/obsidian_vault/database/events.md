...
7. **Event Cascade Delete**: When an entire event is deleted, the `eventDeleteProcessor` Cloud Function fires automatically. It deletes all associated `bills` (each deletion re-triggers `ledgerProcessor` to reverse `balances` footprints), deletes the `event_balances` cache document, and removes all `eventInvitations` for the event — leaving no orphaned documents behind. See [[../backend/cloud_functions|Cloud Functions]] for details.
...
Just like `balances` tracks global debt between two friends over time, `event_balances` securely tracks the isolated mathematical debt *for a single event*.
...
When a user settles up from the Event UI, the `settlementProcessor` handles applying the actual money transfers directly to the global `balances` ledger. The `ledgerProcessor` then auto-fires to rebuild the `event_balances` cache so the event debt mathematically zeroes out.
