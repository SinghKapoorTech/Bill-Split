...
| `isAirbnb`             | Boolean (Optional) | Flag indicating this is an Airbnb/house-sharing bill. Enables the specialized Airbnb wizard flow (dates, fees, night assignments).                                                                                    |
| `paidById`             | String (Optional)  | Ref to **[Users](users.md)** or bill-local person ID. Overrides `ownerId` as the creditor for balances.                                                                                                               |
| `processedBalances`    | Object (Optional)  | Map of `{ [Firebase UID]: amountOwedToCreditor }`. Tracking footprint of what was explicitly added to the global `balances` ledger. Used by the Idempotent Delta engine to perfectly reverse math on re-edits. |
...
>
> Only `user-{uid}` people whose raw Firebase UID matches someone in the owner's `friends` list are written to **[[balances]]** when the bill is finalized.
...
Present only when `isAirbnb === true`. Stores data used by the Airbnb wizard (see [[../ui/airbnb_flow|Airbnb Flow]]).

| Sub-Field | Type | Description |
|---|---|---|
| `airbnbData.checkIn` | Timestamp | Check-in date |
| `airbnbData.checkOut` | Timestamp | Check-out date |
| `airbnbData.totalCost` | Number | Base stay cost in USD |
| `airbnbData.fees` | Array of `{ id, name, amount }` | Additional fees (cleaning, service, pet, etc.) |
| `airbnbData.splitMethod` | `'even'` \| `'byNight'` | How the cost is divided |
| `airbnbData.nightAssignments` | Map of `{ nightKey: personId[] }` | For `'byNight'` mode — which guests stayed each night |
| `airbnbData.feeAssignments` | Map of `{ feeId: personId[] }` | For `'byNight'` mode — which guests share each fee |
...
Since the client no longer directly writes to `balances` or `event_balances`, the bill's balance lifecycle is entirely managed by the backend **`ledgerProcessor` Cloud Function**. (See [[../backend/cloud_functions|Cloud Functions Architecture]] for full details).
...
3. In a single, idempotent transaction, the backend engine strictly reverses the math stored in `processedBalances` from the user's global `balances`, applies the new calculated totals, and updates `processedBalances` back onto the bill.
4. If this is an event bill, the pipeline automatically triggers a rebuild of the `event_balances` cache.
5. The UI updates seamlessly via `onSnapshot` listeners hooked to the `balances` ledger.
...
3. The server natively reverses the exact footprint recorded in `processedBalances` from the `balances` ledger.
...
References **[Users](users.md)**, **[Events](events.md)**, **[Squads](squads.md)**, and **[[balances]]**.
